    ...
    getFacultyById: async (facultyId: string) => {
        try {
            ...
            const { data: faculty, error: facultyError } = await supabase
                .from('profiles')
                .select('id, first_name, last_name, email, phone, avatar_url, bio, account_verified, is_suspended, admin_note, qualification, job_title, created_at, document_verifications(id, document_type, document_url, created_at)')
                .eq('role', 'FACULTY')
                .eq('id', facultyId)
                .single();
